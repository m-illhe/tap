...
double tsp_mst(point *V, int n, int *P, graph T) {
  ...
  int size = ((n-1)*n)/2 ;
  edge *arrayedge = malloc(sizeof(edge)*(size));
  edge new;
  int part = 0;
  for( int i = 0; i < n-1; i++){
    for( int j = i + 1; j < n; j++){
      new.u = i;
      new.v = j;
      new.weight = dist(V[P[i]], V[P[j]]);
      arrayedge[part] = new;
      part = part + 1;
    }
  }
  qsort(arrayedge, size, sizeof(edge), compEdge);
  //printf("jusque la c'est good");
  ...
  int number_edge = 0;
  part = 0;
  while(number_edge < n - 1 ){
    edge arret = arrayedge[part];
    int parent_u = Find(arret.u, parent);
    int parent_v = Find(arret.v, parent);
    if (parent_u == parent_v){
       //printf("arret non valide %d\n", part);
       part = part + 1;
       continue;
    }
    addEdge(T, arret.u, arret.v);
    Union(parent_u, parent_v, parent, rank);
    number_edge = number_edge + 1;
    //printf("arret valide %d\n", part);
    part = part + 1;
  }
